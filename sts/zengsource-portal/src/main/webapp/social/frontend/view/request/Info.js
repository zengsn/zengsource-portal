Ext.define('JXP.social.frontend.view.request.Info', {
    extend: 'Ext.panel.Panel',
    alias : 'widget.socialrequestinfo',
    //id: 'img-detail-panel',
    ...
    tpl: [
        '<div class="details">',
            '<tpl for=".">',
                    (!Ext.isIE6? '<img src="'+ROOT_PATH+'{origAvatar}" />' : 
    ...
    constructor: function(config) {
    	...
    	this.config = Ext.apply(config, {
    		html: '<p style="margin:30px;text-align:center;color:gray;">点击用户头像查看信息</p>',
            buttonAlign: 'center',
            buttons: [{
				//scale: 'medium',
				text: '接受',
    ...
    clearRecord : function() {
        this.body.hide();
        this.body.update('<p style="margin:30px;text-align:center;color:gray;">点击用户头像查看信息</p>');
        this.body.slideIn('l', {
            duration: 250
        });
        var bbar = this.getDockedItems()[0];
